...
from nyt_games.solvers.wordle.WordleSolver import WordleSolver
...
class FilterSolver(WordleSolver):
    ...
    def update_mask(self, guess_word: str, guess_score: list[int]):
        ...
        for letter_score, letter, letter_col in zip(
            guess_score, guess_word, letter_cols
        ):
            ...
            if letter_score != 2:
                ...
                if letter_score == 1:
                    mask = mask & (
                        self.letter_masks[letter]["occurance_sum"]
                        >= non_zero_occurance_count
                    )
                else:
                    mask = mask & (
                        self.letter_masks[letter]["occurance_sum"]
    ...
    def calculate_letter_masks(guess_words, letter_cols) -> dict[pd.Series]:
        ...
        for letter in all_letters:
            letter_masks[letter] = {}
            letter_masks[letter]["occurance_sum"] = pd.Series(
                data=0, index=guess_words.index
            )
            for letter_col in letter_cols:
                mask = guess_words[letter_col] == letter
                letter_masks[letter][letter_col] = mask
                letter_masks[letter]["occurance_sum"] = (
                    letter_masks[letter]["occurance_sum"] + mask
                )
