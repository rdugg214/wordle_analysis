...
def create_hints_table(answers:pd.Series):
    ...
    hints_table = hints_table.groupby(["first_letter", "length"])["answers"].count().unstack()
...
def prep_words():
    # loading of dataset
    all_words = pd.read_csv("datasets/words_spelling_bee.txt", delimiter=" ", header=None)[0]
...
def get_answers(all_words:pd.Series, central_letter:str, spelling_bee_letters:set):
    ...
    valid_answer_mask = unique_characters.apply(lambda x: x.issubset(spelling_bee_letters))
...
def solve_get_hints(all_words:pd.Series, central_letter:str, spelling_bee_letters:set):
